...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['title'], $_FILES['image'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['tmp_name']) {
        $upload_dir = __DIR__ . '/../images/infrastructure/';
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
            $image_path = 'images/infrastructure/' . $img_name;
        }
    }

    if ($image_path) {
        $stmt = $conn->prepare("INSERT INTO infrastructure (title, alt_text, image_path) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $title, $alt_text, $image_path);
        
        if ($stmt->execute()) {
            echo "<script>alert('Image uploaded to infrastructure successfully!'); window.location.href='my-infrastructure.php';</script>";
            exit;
        } else {
...
<main class="main-content">
    <div class="container-fluid">
        <h3 class="mb-4 fw-bold">Upload Infrastructure Image</h3>
            ...
            <div class="card-body p-4">
                <form method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <a href="my-infrastructure.php" class="btn btn-light text-muted">Cancel</a>
